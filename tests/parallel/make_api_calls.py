...
def execute_parallel_requests(url, headers, num_requests):
    def send_request(_):
        try:
...
# Example usage:
url = 'http://localhost:8080/test_auth'
headers = {
...
results = execute_parallel_requests(url, headers, num_requests)

# Print the results
for idx, (status_code, response_text) in enumerate(results, start=1):
    if status_code is not None:
        print(f"Request {idx}: Status Code {status_code} - {response_text}")
